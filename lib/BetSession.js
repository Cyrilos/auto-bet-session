...
class BetSession {
  ...
  async autoBet(betAmount, multiplier, option = "higher") {
    ...
    while(this.isRunning) {
      // betting
      await bet(this.currentBetAmount, this.currentMultiplier, (err, response) => {
        // request successfull
        if(response) {
          ...
          if(resultAmount > 1) {
            this.winStreak++;
            this.lossStreak = 0;
            runBasicHandlers(this.winHandlers);
            runStreakOfWinHandlers(); // check win streak
          }
          // on loss
          else {
            this.winStreak = 0;
            this.lossStreak++;
            runBasicHandlers(this.lossHandlers);
            runStreakOfLossHandlers(); // check loss streak
          }
        }
        // request fails
        else {
        ...
        runBasicHandlers(this.betHandlers)
        this.count++;
      }, this.currentOption);
    }
  }
